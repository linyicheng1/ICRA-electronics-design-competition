...
class GlobalPlannerClient{
 public:
  GlobalPlannerClient():global_planner_actionlib_client_("global_planner_node_action", true),
    ...
    goal_sub_ = rviz_nh.subscribe<geometry_msgs::PoseStamped>("move_base_simple/goal", 1,
                                                              &GlobalPlannerClient::GoalCallback,this);

    my_sub_ =   rviz_nh.subscribe<geometry_msgs::PoseStamped>("myGoal", 10, &GlobalPlannerClient::myGoalCallBack,this);
  ...
  void GoalCallback(const geometry_msgs::PoseStamped::ConstPtr & goal){
    ...
    command_.goal = *goal;
    std::cout<<command_.goal;
    new_goal_ = true;
  }
  void myGoalCallBack(const geometry_msgs::PoseStamped::ConstPtr & goal)
  {
...
//      for(int i=0;i<feedback->path.poses.size();i++)
//      {
//        std::cout << "get :" << feedback->path.poses.at(i) << " ";
//      }
